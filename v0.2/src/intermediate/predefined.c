...
static void define_fun(
	...
	const void *fun_value = new(FunValue, FUNCTION_TYPE_FUN, fun_symbol, fun_symbol_table, param_count, param_symbols, executor_fun, NULL);
...
static void *execute_out(const void *fun, void **args) {
	print_value(args[0]);
	// not deleteing cuz it gets deleted when the frame is popped of
	// delete(args[0]);
