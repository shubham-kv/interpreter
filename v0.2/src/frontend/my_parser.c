...
void *val_init(
	...
	// TODO: addedd enum SymbolAttributeKey
	// wannned ro use it at akk things
...
void *fun_decl(struct Parser *self, enum DefinitionType def_type) {
	...
	if(P_MATCH(self, TOKEN_TYPE_IDENTIFIER)) {
		...
		p_raise_error(
			self, fun_token,
			"SyntaxError: Expected some name after '%.*s'", fun_token->length, fun_token->start
		);
...
//	class_decl	->  "class" IDENTIFIER ( ':' IDENTIFIER )? "{" var_decl* | val_decl* | fun_decl* "} ;
...
void *class_decl(struct Parser *self) {
	...
	if(P_MATCH(self, TOKEN_TYPE_COLON)) {
		...
		if(P_MATCH(self, TOKEN_TYPE_IDENTIFIER)) {
			...
			if(
				...
				memcmp(class_id_token->start, super_class_id_token->start, super_class_id_token->length)
			) {
			...
			void *super_class_symbol = symtabstack_lookup(self->symbol_table_stack, super_class_id);
	...
	char *instance_ptr = allocate(sizeof(char), len);
	...
	if(NULL != super_class_symbol) {
		...
		char *super_ptr = allocate(sizeof(char), len);
		strncpy(super_ptr, super, len);
	...
	while(!p_match_any(self, 2, TOKEN_TYPE_RCURLY_BRACE, TOKEN_TYPE_EOF)) {
		...
		// }
			...
			struct FunDecl *fundecl = (struct FunDecl *) fun_decl(self, DEF_TYPE_METHOD);

			// TODO: want to support method polymorphism
			methods = reallocate(methods, sizeof(void *), ++method_count);
			methods[method_count - 1] = fundecl;
		}
	...
	#if defined(DEBUG)
		vi_log("ret class_decl()\n");	
	#endif // DEBUG

	return new(
		ClassDecl,
		class_symbol, class_symbol_table, super_class_symbol,
		property_count, properties, methods
	);
}
...
void *unary(struct Parser *self) {
	...
	if(p_match_any(self, 2, TOKEN_TYPE_PLUS_PLUS, TOKEN_TYPE_MINUS_MINUS)) {
		...
		if(id_symbol->definition_type != DEF_TYPE_VAR) {
			p_raise_error(
				self, id_token,
	...
	while(1) {
		if(P_MATCH(self, TOKEN_TYPE_LPAREN)) {
			...
			if(first_time) {
				...
				if((DEF_TYPE_VAR != def_type) && (DEF_TYPE_VAL != def_type) && (DEF_TYPE_CUR_INSTANCE_POINTER != def_type)) {
					p_raise_error_expr(
						self, left_expr,
						"Error: Target element is not an object."
					);
			...
			#if defined(DEBUG)
				vi_log("molecule(): left is set to a GetExpr\n");
			#endif // DEBUG
		}
...
void *atom(struct Parser *self) {
	...
	if(
		P_MATCH(self, TOKEN_TYPE_INTEGER) ||
		P_MATCH(self, TOKEN_TYPE_DOUBLE) ||
		P_MATCH(self, TOKEN_TYPE_STRING) ||
		P_MATCH(self, TOKEN_TYPE_TRUE) ||
		P_MATCH(self, TOKEN_TYPE_FALSE)
	) {
		p_advance(self);
