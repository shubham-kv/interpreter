...
static const struct Type _type_ast_visitor = {
	sizeof(struct AstVisitor),
	ast_visitor_constructor,
	ast_visitor_destructor
};
...
static void *ast_visitor_constructor(void *_self, va_list *ap) {
	...
	self->visit_print_stmt 	= va_arg(*ap, visitor_fun);
	...
	// self->visit_assign_expr 	= va_arg(*ap, visitor_fun);
