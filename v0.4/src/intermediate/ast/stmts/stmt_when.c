...
static void *when_stmt_constructor(void *self, va_list *ap);
static void *when_stmt_destructor(void *self);

static void *when_stmt_accept_visitor(const void *self, const void *ast_visitor);
static bool is_when_stmt(const void *object);

/*
static const struct AstNodeType _type_when_stmt = {
	sizeof(struct WhenStmt),
	when_stmt_constructor,
	when_stmt_destructor,
	when_stmt_accept_visitor,
	is_when_stmt
};
// */

static const struct AstNodeType _type_when_stmt = {
	.size 					= sizeof(struct WhenStmt),
	.constructor 			= when_stmt_constructor,
	.destructor 			= when_stmt_destructor,
	.accept_visitor 		= when_stmt_accept_visitor,
	.is_ast_node 			= is_when_stmt,
...
static void *when_stmt_constructor(void *_self, va_list *ap) {
...
static void *when_stmt_destructor(void *_self) {
	struct WhenStmt *self = _self;
	assert(IS_WHEN_STMT(self));
...
static void *when_stmt_accept_visitor(const void *_self, const void *_ast_visitor) {
