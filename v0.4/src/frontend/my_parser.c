...
void *bitwise_or(struct Parser* self);
void *bitwise_xor(struct Parser* self);
void *bitwise_and(struct Parser* self);
...
void *bitwise_or(struct Parser* self) {
    ...
    void* left = bitwise_xor(self);

    if (NULL == left)
        return NULL;

    while (P_MATCH(self, TOKEN_TYPE_BITWISE_OR)) {
        const struct Token* op_token = CUR_TOKEN(self);
        P_ADVANCE(self);

        void* right = bitwise_xor(self);
        ...
        left = new(BitwiseExpr, op_token, left, right);
    }

	#if defined(DEBUG)
		vi_log("ret bitwise_or()\n");
	#endif // DEBUG
...
void *bitwise_xor(struct Parser* self) {
    ...
    void* left = bitwise_and(self);

    if (NULL == left)
        return NULL;

    while (P_MATCH(self, TOKEN_TYPE_BITWISE_XOR)) {
        const struct Token* op_token = CUR_TOKEN(self);
        P_ADVANCE(self);

        void* right = bitwise_and(self);
...
void *bitwise_and(struct Parser* self) {
    ...
    while (P_MATCH(self, TOKEN_TYPE_BITWISE_AND)) {
        const struct Token* op_token = CUR_TOKEN(self);
        P_ADVANCE(self);
...
void* unary(struct Parser* self) {
    ...
    if (p_match_any(self, 4, TOKEN_TYPE_MINUS, TOKEN_TYPE_PLUS, TOKEN_TYPE_NOT, TOKEN_TYPE_BITWISE_NOT)) {
        const struct Token* op_token = CUR_TOKEN(self);
        P_ADVANCE(self);
