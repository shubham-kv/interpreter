...
static void *next_token(void *_self) {
	...
	switch(c) {
		...
		case '&': {
			...
			return make_token(self, TOKEN_TYPE_BITWISE_AND, 1);
		}
		...
		case '|': {
			...
			return make_token(self, TOKEN_TYPE_BITWISE_OR, 1);
		}

		case '^':
			return make_token(self, TOKEN_TYPE_BITWISE_XOR, 1);

		case '~':
			return make_token(self, TOKEN_TYPE_BITWISE_NOT, 1);
